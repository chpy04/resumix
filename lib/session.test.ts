...
test('password mode rejects a validly-signed token naming a user that does not exist', { skip }, async () => {
...
test('supabase mode fails closed — it never lets a request through unidentified', { skip }, async () => {
...
test('loginWithPassword refuses to guess when several users exist and OWNER_EMAIL is unset', { skip }, async () => {
