...
| id | task | owns | branch |
|---|---|---|---|
| T1 | Drizzle schema + SQL migrations + `npm run db:migrate` | `lib/db/**`, `drizzle/**` | `feat/t1-db-schema` |
| T2 | LaTeX compile service (Dockerfile + Express) + `lib/latex.ts` client | `services/latex/**`, `lib/latex.ts` | `feat/t2-latex-service` |
| T3 | Render engine: selections + template → `.tex`, unit-tested | `lib/render/**` | `feat/t3-render-engine` |
| T4 | Auth: password → token, middleware, login screen | `lib/auth.ts`, `middleware.ts`, `app/login/**` | `feat/t4-auth` |
