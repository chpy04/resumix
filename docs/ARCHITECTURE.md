...
| Multi-tenancy | `user_id` on the five root tables; children inherit through their parent; every query takes a `userId` | one source of truth per fact (D-017) |
| Auth | three modes behind one `requireUserId()`: `dev` (auto-login as the seeded user), `password` (shared password → HMAC token naming a user), `supabase` (OAuth, stubbed) | local dev needs no credentials; production keeps a real gate (D-018) |
